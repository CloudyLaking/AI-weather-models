    ...
    def process_era5_data(self, datetime_str, skip_existing=True):
        ...
        try:
            ...
            if not os.path.exists(single_level_file):
                legacy_single = os.path.join(self.raw_input_dir, f'era5_surface_{datetime_str}.nc')
                if os.path.exists(legacy_single):
                    print(f"  [LOAD] Using existing legacy surface file: {legacy_single}")
                    single_level_file = legacy_single
                else:
                    request = {
            ...
            if not os.path.exists(pressure_file):
                legacy_upper = os.path.join(self.raw_input_dir, f'era5_upper_{datetime_str}.nc')
                if os.path.exists(legacy_upper):
                    print(f"  [LOAD] Using existing legacy upper file: {legacy_upper}")
                    pressure_file = legacy_upper
                else:
                    request = {
                        'product_type': 'reanalysis',
                        'variable': [
