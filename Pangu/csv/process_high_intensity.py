...
def find_single_typhoons(csv_path):
    ...
    output_folder = "high_intensity_csv"
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    
    # 对每个台风，检查是否最低气压达到 940hPa及以下
    for sid, info in typhoon_dict.items():
        data = info['data']
        # 检查USA_PRES（后续会重命名为WMO_PRES）是否达到940hPa及以下
        in_pressure = data[data['USA_PRES'] <= 940]
        if in_pressure.empty:
            continue  # 不满足条件
            
        # 获取气压达到940hPa及以下时的最早和最晚记录时间
        region_start = in_pressure['ISO_TIME'].min()
        region_end = in_pressure['ISO_TIME'].max()
        # 扩展时间区间：最早记录前2天，最晚记录后3天
