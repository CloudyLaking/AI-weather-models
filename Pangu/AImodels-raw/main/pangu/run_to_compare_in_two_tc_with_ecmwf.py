...
def process_workflow(csv_file):
    ...
    针对单个 CSV 文件执行流程：
      - 下载初始场数据（ERA5/GFS）
      - 运行 Pangu 模型预报（输出图像保存在 output_image_path 下），同时获取文件字典
      - 下载 ECMWF 预报数据
      - 合并提取结果并输出新 CSV
    """
    global current_csv
    ...
    print(f"处理文件 {csv_file}，台风起始时刻: {base_time}")
    ...
    files_dict = run_pangu(6, data_source, run_times=num_steps, base_datetime=base_time.strftime("%Y%m%d%H"), output_image_path=output_image_path)
...
def process_all_csv():
    ...
    """
    TY_PATH = os.path.join("two-typhoon-csv")
...
def main():
    ...
    lat_min = 0
    lat_max = 35
    globals()['lon_min'] = lon_min
    globals()['lon_max'] = lon_max
    globals()['lat_min'] = lat_min
    globals()['lat_max'] = lat_max
